...
test('all frontend-address write paths serialize registration before storage changes', () => {
  const setWebuiStart = background.indexOf('function setWebuiUrl');
  const setWebuiEnd = background.indexOf('async function getLiveActiveTabId', setWebuiStart);
  const setWebuiBlock = background.slice(setWebuiStart, setWebuiEnd);
  assert.ok(
    setWebuiBlock.indexOf('await prepareWebuiContentScripts(webuiUrl)')
      < setWebuiBlock.indexOf('await chrome.storage.local.set({ webuiUrl })')
  );

  const setStateStart = background.indexOf("if (message.type === 'NEKO_SET_STATE')");
  const setStateEnd = background.indexOf("if (message.type === 'NEKO_SET_SURFACE_COMPONENTS')", setStateStart);
  const setStateBlock = background.slice(setStateStart, setStateEnd);
  assert.match(setStateBlock, /if \(payload\.webuiUrl\) \{\s*await setWebuiUrl\(payload\.webuiUrl\)/);
  assert.ok(
    setStateBlock.indexOf('await setWebuiUrl(payload.webuiUrl)')
      < setStateBlock.indexOf('await chrome.storage.local.set(payload)')
  );
  assert.match(setStateBlock, /delete payload\.webuiUrl/);
  assert.match(setWebuiBlock, /return queueWebuiUrlTransition\(async \(\) =>/);

  const installStart = background.indexOf('chrome.runtime.onInstalled.addListener');
  const installEnd = background.indexOf('chrome.runtime.onStartup.addListener', installStart);
  const installBlock = background.slice(installStart, installEnd);
  assert.match(installBlock, /await queueWebuiUrlTransition\(async \(\) =>/);

  const claimStart = background.indexOf('async function claimSidePanel');
  const claimEnd = background.indexOf('async function releaseSidePanel', claimStart);
  const claimBlock = background.slice(claimStart, claimEnd);
  assert.match(claimBlock, /await prepareWebuiContentScripts\(\)/);
  assert.doesNotMatch(claimBlock, /prepareWebuiContentScripts\(state\.webuiUrl\)/);
  assert.match(
    background,
    /return queueWebuiUrlTransition\(async \(\) => \{\s*const stored = await getStoredState\(\)/
  );
  assert.match(setWebuiBlock, /catch \(transitionError\)[\s\S]*prepareWebuiContentScripts\(persistedWebuiUrl\)/);
});
